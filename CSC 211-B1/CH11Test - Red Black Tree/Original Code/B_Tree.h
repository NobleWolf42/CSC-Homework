...
*/
  ...
  */
  const Item_Type* find(const Item_Type& item) {
    /*<exercise chapter="11" section="5" type="programming" number="1">*/
  ...
  */
  const Item_Type* find(Node* local_root, const Item_Type& item) {
    ...
    Item_Type* pointer = binary_search(local_root->data,
				       local_root->data + local_root->size);
    size_t index = pointer - local_root->data;
    if (index < local_root->size && local_root->data[index] == item) {
      return pointer;
    } else {
      return find(local_root->child[index], item);
    }
    /*</exercise>*/
  }
  ...
  */
  const Item_Type* binary_search(Item_Type* first, Item_Type* last, 
    ...
    /*<exercise chapter="11" section="5" type="progrmming" number="2">*/
    if (last == first) return first;
    if ((last - first) <= 1) {
      if (item <= *first) return first;
      else return last;
    }
    Item_Type* middle = first + (last - first)/2;
    if (*middle == item) return middle;
    if (item < *middle) 
      return binary_search(first, middle, item);
    else
      return binary_search(middle + 1, last, item);
    /*</exercise>*/
  }
